...
function LoginContent({ googleClientId }: { googleClientId: string }) {
    ...
    const handleContinue = async () => {
        ...
        try {
            ...
            await signIn(
                "google",
                { callbackUrl: "/user" },
                { prompt: "none", login_hint: storedUser.email }
            )
        } catch {
            setIsLoggingIn(false)
        }
...
export default function LoginPageClient({ googleClientId }: { googleClientId: string }) {
    return (
        <Suspense fallback={
            <div className="min-h-screen flex items-center justify-center bg-muted/30 relative overflow-hidden">
                <LoginBg />
            ...
            <LoginContent googleClientId={googleClientId} />
        </Suspense>
    )
}
