...
import {
    Calendar, Clock, LogIn, LogOut, Coffee, FileText, Settings, ShieldCheck,
    AlertCircle, History, UserCheck, ShieldAlert, Edit2, Trash2, ListFilter,
    ChevronDown, ChevronRight, LayoutList, Search, Users, Eye
} from "lucide-react"
...
export default function AdminUserActivityPage() {
    ...
    const employees: any[] = useMemo(() => {
        ...
        return Array.isArray(employeesData) ? employeesData : (employeesData.employees || [])
    }, [employeesData])

    const { data, isLoading } = useSWR(
        selectedUserId ? `/api/activity-logs?limit=500&userId=${selectedUserId}` : null,
        fetcher
    )
    ...
    const filteredLogs = useMemo(() => {
        return logs.filter((log: any) => {
            ...
            if (action.startsWith('LEAVE_') || action.startsWith('ATTENDANCE_REQUEST_')) mappedGroup = 'LEAVES'
            else if (action.startsWith('ADMIN_') || action.startsWith('ATTENDANCE_SUMMARY')) mappedGroup = 'ADMIN'
    ...
    const toggleFilter = (id: string) => setSelectedFilters(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])
    ...
    const userInitials = selectedUser?.name?.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) || '?'
    ...
    return (
        <div className="space-y-8 w-full p-4 lg:p-8 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b pb-6">
                <div>
                    <h1 className="text-3xl font-black tracking-tighter text-[#8B2323] uppercase">User Activity</h1>
                    <p className="text-red-900/60 mt-1 font-bold text-sm tracking-widest uppercase">Staff Activity Logs</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <Popover open={staffPickerOpen} onOpenChange={setStaffPickerOpen}>
                        <PopoverTrigger asChild>
                            <Button variant="outline" className="h-10 min-w-[200px] justify-between border-slate-200 shadow-sm rounded-xl font-bold bg-white text-slate-700 px-4">
                                <div className="flex items-center gap-2">
                                    {selectedUser ? (
                                        <>
                                            <div className="w-5 h-5 rounded-full bg-[#8B2323] text-white text-[9px] font-black flex items-center justify-center flex-shrink-0">
                                                {userInitials}
                                            </div>
                                            <span className="truncate max-w-[140px]">{selectedUser.name}</span>
                                        </>
                                    ) : (
                                        <>
                        ...
                        <PopoverContent className="w-72 p-3 rounded-2xl bg-white/95 backdrop-blur-md shadow-2xl border-slate-100" align="end">
                            <div className="space-y-2">
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        className="pl-8 h-8 text-sm border-slate-200 rounded-lg"
                                        autoFocus
                                    />
                                </div>
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                selectedUserId === emp.id
                                                    ? "bg-red-50 text-[#8B2323]"
                                                    : "hover:bg-slate-50 text-slate-700"
                                            )}
                                        >
                                            <div className={cn(
                                                "w-7 h-7 rounded-full text-[10px] font-black flex items-center justify-center flex-shrink-0",
                                                selectedUserId === emp.id ? "bg-[#8B2323] text-white" : "bg-slate-200 text-slate-500"
                                            )}>
                        ...
                        <PopoverContent className="w-64 p-3 rounded-2xl bg-white/95 backdrop-blur-md shadow-2xl border-slate-100" align="end">
                            <div className="space-y-3">
                                <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                                    ...
                                    <Button variant="ghost" size="sm" onClick={() => setSelectedFilters([])} className="h-6 text-[10px] uppercase font-bold text-[#8B2323] hover:text-[#701c1c] hover:bg-red-50 px-2 rounded-md">
                                        Clear
                                    </Button>
                                </div>
                                <div className="grid gap-2">
                                    {FILTER_OPTIONS.map(option => (
                                        <label key={option.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors group">
                                            ...
                                            />
                                            <span className="text-sm font-bold text-slate-700 select-none flex-1">{option.label}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
            ...
            {!selectedUserId && (
                <Card className="rounded-3xl border-dashed border-2 border-slate-200 shadow-none bg-slate-50/30">
                    <CardContent className="p-20 text-center">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tight">Select a Staff Member</h3>
                        <p className="text-slate-500 mt-2 font-medium">Choose a staff member above to view their activity logs.</p>
                    </CardContent>
                </Card>
            )}
                ...
                <div className="space-y-4">
                    {!isLoading && sortedDates.length > 0 && (
                        <div className="flex items-center justify-between px-2 mb-2">
                            <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-full bg-[#8B2323] text-white text-xs font-black flex items-center justify-center">
                                    {userInitials}
                                </div>
                                <span className="text-sm font-black text-slate-700">{selectedUser?.name}</span>
                    ...
                    {isLoading ? (
                        ...
                        <Card className="rounded-3xl border-dashed border-2 border-slate-200 shadow-none bg-slate-50/30">
                            <CardContent className="p-20 text-center">
                                ...
                                <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tight">No Activity Found</h3>
                                <p className="text-slate-500 mt-2 font-medium">No logs for {selectedUser?.name} in this date range.</p>
                            </CardContent>
                        </Card>
